using CustomerApp.Models;
using CustomerApp.Repositories;
using System;
using System.Collections.Generic;
using System.Net.Mail;
using System.Threading.Tasks;

namespace CustomerApp.Services;

public class CustomerService(ICustomerRepository customerRepository) : ICustomerService
{
    public async Task CreateCustomerAsync(string name, string email, bool isCompany)
    {
        if (string.IsNullOrWhiteSpace(name))
            throw new ArgumentException(isCompany ? "Ange företagets namn." : "Ange kundens namn.");

        if (string.IsNullOrWhiteSpace(email))
            throw new ArgumentException("Ange en e-postadress.");

        name = name.Trim();
        email = email.Trim();

        if (!MailAddress.TryCreate(email, out var address) || address.Address != email)
            throw new ArgumentException("Ange en giltig e-postadress.");

        var customers = await customerRepository.GetAllAsync();

        var customer = new Customer
        {
            Name = name,
            Email = email,
            IsCompany = isCompany
        };

        customer.SetId(Guid.NewGuid());

        customers.Add(customer);

        await customerRepository.SaveAllAsync(customers);
    }

    public async Task DeleteCustomerAsync(Guid customerId)
    {
        var customers = await customerRepository.GetAllAsync();

        var removedCount = customers.RemoveAll(customer => customer.Id == customerId);

        if (removedCount > 0)
            await customerRepository.SaveAllAsync(customers);
    }

    public async Task<List<Customer>> GetAllCustomersAsync()
    {
        return await customerRepository.GetAllAsync();
    }
}
